...
for k in range(1000000):
    ...
    def objective(trial, x_train, y_train, x_test, y_test, min_rmse):
        alpha = trial.suggest_loguniform('alpha', 0.00005, 0.0008)
        n_restarts_optimizer  = trial.suggest_int('n_restarts_optimizer', 5, 10)
        ...
        print('GPR RMSE : ', rmse)
        if rmse < 0.302:
            submit_csv['Calories_Burned'] = np.round(model.predict(test_csv),2)
