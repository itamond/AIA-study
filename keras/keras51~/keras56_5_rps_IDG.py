...
xy_train =train_datagen.flow_from_directory(                     #폴더에서 가져올거야~    
    'd:/study_data/_data/rps/',                #이미지제너레이터는 폴더별로 라벨값 부여. 때문에 분류 폴더 이전 상위폴더까지만 설정해도됨
...
np.save(save_path + 'keras56_5_x_train.npy', arr=xy_train[0][0])          #수치화된 데이터를 np형태로 저장
# np.save(path + 'keras56_x_test.npy', arr=xy_test[0][0])    
np.save(save_path + 'keras56_5_y_train.npy', arr=xy_train[0][1])    
