...
x_submit = test_input_dataset[test_input_dataset.isna().any(axis=1)] # 결측된 행만 추출
...
y_predict = model.predict(x_submit)
...
submission = pd.read_csv(path+'answer_sample.csv', index_col=0, header=0, encoding='utf-8-sig')
submission['PM2.5'] = y_predict
submission.to_csv(path+'sample1.csv',index = None)
