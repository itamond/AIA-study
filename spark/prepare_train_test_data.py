...
def save_concated_data_min_distance(print_download:bool=True)->None:
    ...
    for i in range(len(train_datas)):
        ...
        os.makedirs('./03.AI_finedust', exist_ok=True)
        os.makedirs('./03.AI_finedust/for_Train', exist_ok=True)
        os.makedirs('./03.AI_finedust/for_Train/test/', exist_ok=True)
        os.makedirs('./03.AI_finedust/for_Train/train/', exist_ok=True)  
        train_datas[i].to_csv(f'./03.AI_finedust/for_Train/train/{pm_name[i]}.csv',index=False)
        test_datas[i].to_csv(f'./03.AI_finedust/for_Train/test/{pm_name[i]}.csv',index=False)
        if print_download:
...
#     train_datas,test_datas,pm_name=load_min_distance(print_download=print_download,load_name=True)
#     for i in range(len(train_datas)):
#         train_datas[i]=pd.read_csv(f'./03.AI_finedust/for_Train/train/{pm_name[i]}.csv')
#         test_datas[i]=pd.read_csv(f'./03.AI_finedust/for_Train/test/{pm_name[i]}.csv')
#     if print_download:
