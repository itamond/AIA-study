...
# train: x
x = {}
train = os.scandir('./_data/aif/TRAIN/')
train_aws = os.scandir('./_data/aif/TRAIN_AWS/')
...
for file in region.keys():
    x = {}
    pm_datapath = './_data/aif/TRAIN/' + unicodedata.normalize('NFC', file) + '.csv'
    pm_data = pd.read_csv(pm_datapath, index_col=False)
    aws_datapath = './_data/aif/TRAIN_AWS/' + unicodedata.normalize('NFC', region[file]) + '.csv'
    ...
    df_x.to_csv(f'./_data/aif/NTRAIN/{file}.csv', index=False)